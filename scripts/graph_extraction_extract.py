...
def extract_data(config):
    ...
    for y in config["scenario"]["planning_horizons"]:
        run_name = Path(config["path"]["n_path"], "postnetworks", config["n_name"] + f"{y}.nc")
    ...
    n_bf = pypsa.Network(Path(config["path"]["n_path"], "prenetworks-brownfield", config["n_name"] + f"{2030}.nc"))
